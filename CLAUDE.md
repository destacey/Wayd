...
- The tree is marked `linguist-generated=true` in `.gitattributes` so GitHub collapses it in PR diffs and Copilot code review drops it from its per-PR file budget (a single handler change regenerates 400+ files, which otherwise exceeds Copilot's limit and blocks review). This affects diff *display* only — the files stay committed and tracked, and the CI staleness check still runs. If Copilot still loads them, add a content-exclusion for the same glob in repo Settings → Copilot.
