...
/// <summary>
/// Submits a file of employees to import, and answers with the id of the run.
/// </summary>
/// <remarks>
...
/// file may span, or an authorization question the permission claim alone cannot answer. Employees have
/// none today, which is why this handler is thin; it is the seam that matters, not its current contents.
/// </para>
/// </remarks>
public sealed record ImportEmployeesCommand(IReadOnlyList<SubmittedImportRow<ImportEmployeeDto>> Rows, Guid? SubmissionGroupId = null, bool ValidateOnly = false) : ICommand<Guid>;
...
public sealed class ImportEmployeesCommandValidator : CustomValidator<ImportEmployeesCommand>
{
    public ImportEmployeesCommandValidator()
    {
        ...
        RuleForEach(c => c.Rows)
            .ChildRules(row => row.RuleFor(r => r.Data).NotNull().SetValidator(new ImportEmployeeDtoValidator()));
    }
}

public sealed class ImportEmployeesCommandHandler(
    IImportDefinitionRegistry registry,
    IDispatcher dispatcher) : ICommandHandler<ImportEmployeesCommand, Guid>
{
    private readonly IImportDefinitionRegistry _registry = registry;
    private readonly IDispatcher _dispatcher = dispatcher;

    public async Task<Result<Guid>> Handle(ImportEmployeesCommand command, CancellationToken cancellationToken)
    {
        if (command.Rows.Count == 0)
            return Result.Failure<Guid>("The file contains no employees.");

        var definition = _registry.Find(EmployeeImportDefinition.ImportKey);
        ...
        return await _dispatcher.Send(
            new SubmitImportCommand(EmployeeImportDefinition.ImportKey, rows, command.SubmissionGroupId, command.ValidateOnly), cancellationToken);
    }
}
