...
public sealed class DeactivateTeamCommandHandler(IOrganizationDbContext organizationDbContext, ICurrentUser currentUser, ILogger<DeactivateTeamCommand> logger, IDateTimeProvider dateTimeProvider) : ICommandHandler<DeactivateTeamCommand>
{
    ...
    public async Task<Result> Handle(DeactivateTeamCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = team.Deactivate(TeamDeactivatableArgs.Create(request.InactiveDate, EventActor.User(_currentUser.GetUserId(), _currentUser.GetEmployeeId()), _dateTimeProvider.Now));
            if (result.IsFailure)
            {
                // Reset the entity
