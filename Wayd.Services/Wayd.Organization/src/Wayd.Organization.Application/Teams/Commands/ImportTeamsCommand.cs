...
/// </remarks>
public sealed record ImportTeamsCommand(IReadOnlyList<SubmittedImportRow<ImportTeamDto>> Rows, Guid? SubmissionGroupId = null, bool ValidateOnly = false) : ICommand<Guid>;
    ...
    IDispatcher dispatcher) : ICommandHandler<ImportTeamsCommand, Guid>
{
    ...
    public async Task<Result<Guid>> Handle(ImportTeamsCommand command, CancellationToken cancellationToken)
    {
        ...
        return await _dispatcher.Send(
            new SubmitImportCommand(TeamImportDefinition.ImportKey, rows, command.SubmissionGroupId, command.ValidateOnly), cancellationToken);
    }
}
