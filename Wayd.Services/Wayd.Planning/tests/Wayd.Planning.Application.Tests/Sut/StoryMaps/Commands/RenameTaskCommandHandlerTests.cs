...
public class RenameTaskCommandHandlerTests : IDisposable
{
    private readonly FakePlanningDbContext _dbContext;
    private readonly RenameTaskCommandHandler _handler;
    private readonly Mock<ILogger<RenameTaskCommandHandler>> _mockLogger;
    private readonly Mock<IStoryMapNotifier> _mockNotifier;

    public RenameTaskCommandHandlerTests()
    {
        _dbContext = new FakePlanningDbContext();
        _mockLogger = new Mock<ILogger<RenameTaskCommandHandler>>();
        _mockNotifier = new Mock<IStoryMapNotifier>();

        _handler = new RenameTaskCommandHandler(_dbContext, _mockNotifier.Object, _mockLogger.Object);
    }
    ...
    [Fact]
    public async Task Handle_ShouldRenameTask_WhenMapExists()
    {
        // Arrange
        var (map, taskId) = CreateMapWithTask();
        _dbContext.AddStoryMap(map);

        var command = new RenameTaskCommand(map.Id, taskId, "Updated title");
    ...
    /// <summary>
    /// The reason this command exists: renaming must not disturb a description set elsewhere.
    /// </summary>
    [Fact]
    public async Task Handle_ShouldLeaveDescriptionUntouched()
    {
        // Arrange
        var (map, taskId) = CreateMapWithTask();
        ...
        var task = map.Goals.SelectMany(g => g.Steps).SelectMany(s => s.Tasks).First(t => t.Id == taskId);
        task.Title.Should().Be("Updated title");
        task.Description.Should().Be("Existing notes");
    }
    ...
    {
        // Arrange
        var command = new RenameTaskCommand(Guid.NewGuid(), Guid.NewGuid(), "Updated title");
    ...
    public async Task Handle_ShouldFail_WhenTaskDoesNotExist()
    {
        ...
        var command = new RenameTaskCommand(map.Id, Guid.NewGuid(), "Updated title");
