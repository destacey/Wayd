...
public class SetChecklistItemCheckedCommandHandlerTests : IDisposable
{
    private readonly FakePlanningDbContext _dbContext;
    private readonly SetChecklistItemCheckedCommandHandler _handler;
    private readonly Mock<ILogger<SetChecklistItemCheckedCommandHandler>> _mockLogger;
    private readonly Mock<IStoryMapNotifier> _mockNotifier;

    public SetChecklistItemCheckedCommandHandlerTests()
    {
        _dbContext = new FakePlanningDbContext();
        _mockLogger = new Mock<ILogger<SetChecklistItemCheckedCommandHandler>>();
        _mockNotifier = new Mock<IStoryMapNotifier>();

        _handler = new SetChecklistItemCheckedCommandHandler(_dbContext, _mockNotifier.Object, _mockLogger.Object);
    }

    [Fact]
    public async Task Handle_ShouldSetCheckedAndNotify_WhenItemExists()
    {
        // Arrange
        ...
        var command = new SetChecklistItemCheckedCommand(map.Id, task.Id, item.Id, true);
    ...
    {
        // Arrange
        var command = new SetChecklistItemCheckedCommand(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), true);
    ...
    public async Task Handle_ShouldFail_WhenTaskNotFound()
    {
        ...
        var command = new SetChecklistItemCheckedCommand(map.Id, Guid.NewGuid(), Guid.NewGuid(), true);
