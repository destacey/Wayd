...
/// <summary>
/// The PPM module's copy of an Organization team, kept by the <c>Team*</c> events (see
...
public sealed class PpmTeam : ISimpleTeam, IHasIdAndKey, IHasTeamIdAndCode
{
    ...
    /// </param>
    public PpmTeam(ISimpleTeam team, Instant asOf)
    {
        Id = team.Id;
        Key = team.Key;
        Name = team.Name;
    ...
    public bool Resync(ISimpleTeam team, Instant asOf)
    {
        if (Id != team.Id)
        {
            throw new InvalidOperationException("Cannot resync PpmTeam with a different Id.");
        }
