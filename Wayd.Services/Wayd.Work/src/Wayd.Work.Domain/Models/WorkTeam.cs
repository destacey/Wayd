...
/// <summary>
/// The Work module's copy of an Organization team, kept by the <c>Team*</c> events (see
...
public sealed class WorkTeam : ISimpleTeam, IHasIdAndKey, IHasTeamIdAndCode
{
    ...
    /// </param>
    public WorkTeam(ISimpleTeam team, Instant asOf)
    {
        Id = team.Id;
        Key = team.Key;
        Name = team.Name;
    ...
    public bool Resync(ISimpleTeam team, Instant asOf)
    {
        if (Id != team.Id)
        {
            throw new InvalidOperationException("Cannot resync WorkTeam with a different Id.");
        }
