...
| **Projects** | List (with role filter, for the caller or another employee), get details, get activity history, get status history, get my involvement summary, get task metrics (mine or a colleague's), get team, get stages, get stage details, get plan tree, get plan summary (single and batch), list health checks, get health check, create health check, get scoring context, list scores, get score, update/delete health check, forecast completion. Create, update, change program, change key. Status: approve, activate, complete, cancel, revert to an earlier status |
