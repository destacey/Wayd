...
export const definitions: [string, McpToolDefinition][] = [
  ...
  [
    'Teams_GetTeam',
    {
      ...
      inputSchema: {
        type: 'object',
        properties: { id: { type: 'number', format: 'int32' } },
        required: ['id'],
      },
      method: 'get',
    ...
    'Teams_GetBacklogHealth',
    {
      ...
      inputSchema: {
        ...
        properties: {
          idOrCode: {
            type: 'string',
            description: 'Team ID (UUID) or team code — not the integer key.',
          },
          lookbackDays: {
            type: 'integer',
            minimum: 14,
            maximum: 365,
            description: 'Days of history for throughput, cycle time and net flow (default 90).',
          },
          staleDays: {
            type: 'integer',
            minimum: 1,
            maximum: 3650,
            description: 'Days without a change before a work item is stale (default 90).',
          },
          oldProposedDays: {
            type: 'integer',
            minimum: 1,
            maximum: 3650,
            description: 'Days since creation before a proposed work item is old (default 180).',
          },
          agingWipPercentile: {
            type: 'integer',
            minimum: 1,
            maximum: 100,
            description: 'Cycle time percentile an active work item is aging beyond (default 85).',
          },
          oversizedPercentile: {
            type: 'integer',
            minimum: 1,
          ...
          readinessWindowWeeks: {
            ...
            minimum: 1,
            maximum: 52,
            description: 'Weeks of throughput the readiness checks look ahead (default 4).',
          },
          readinessFallbackItems: {
            type: 'integer',
            minimum: 1,
            maximum: 1000,
            description:
              'Top-ranked work items the readiness checks look at without enough history (default 20).',
          },
          atRiskPercent: {
            type: 'integer',
            minimum: 1,
            maximum: 100,
            description:
              'Percent of in-scope work items flagged at which a check is At Risk (default 10).',
          },
          unhealthyPercent: {
            type: 'integer',
            minimum: 1,
            maximum: 100,
            description:
              'Percent flagged at which a check is Unhealthy (default 25); not below atRiskPercent.',
          },
          runwayAtRiskWeeks: {
            type: 'number',
            minimum: 0,
            maximum: 520,
            description: 'Runway weeks below which the backlog is At Risk (default 4).',
          },
          runwayUnhealthyWeeks: {
            type: 'number',
            minimum: 0,
            maximum: 520,
            description:
              'Runway weeks below which the backlog is Unhealthy (default 2); not above runwayAtRiskWeeks.',
          },
          runwayTooLongWeeks: {
            type: 'number',
            minimum: 1,
            maximum: 520,
            description:
              'Runway weeks above which the backlog is At Risk for being too long (default 26).',
          },
          netFlowAtRisk: {
            type: 'number',
            minimum: 0.01,
            maximum: 100,
            description:
              'Work items created per completed above which net flow is At Risk (default 1.2).',
          },
          netFlowUnhealthy: {
            type: 'number',
            minimum: 0.01,
            maximum: 100,
            description:
              'Net flow above which it is Unhealthy (default 1.5); not below netFlowAtRisk.',
          },
          wipLoadAtRisk: {
            type: 'number',
            minimum: 0.01,
            maximum: 100,
            description:
              'Active work items per member above which WIP load is At Risk (default 1.5).',
          },
          wipLoadUnhealthy: {
            type: 'number',
            minimum: 0.01,
            maximum: 100,
            description:
              'WIP load above which it is Unhealthy (default 2); not below wipLoadAtRisk.',
          },
        },
        required: ['idOrCode'],
      },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          targetDate: {
            type: 'string',
            format: 'date',
            description: 'The last day to count up to, as YYYY-MM-DD.',
          },
          lookbackDays: {
            type: 'integer',
            minimum: 14,
            maximum: 365,
            description:
              'Days of history, ending yesterday (UTC), to sample throughput from (default 90).',
          },
          startedWorkFirst: {
            type: 'boolean',
            description:
              'Count active backlog items ahead of proposed ones, each in rank order, since teams usually finish what they have started (default true). False orders by rank alone.',
          },
        },
    ...
    'Teams_GetTeamAllocation',
    {
      ...
      inputSchema: {
        ...
        properties: {
          idOrCode: {
            type: 'string',
            description: 'Team ID (UUID) or team code — not the integer ID.',
          },
          from: {
            type: 'string',
            format: 'date',
            description: 'The first day of completed work to include, as YYYY-MM-DD (UTC).',
          },
          to: {
            type: 'string',
            format: 'date',
            description:
              'The last day of completed work to include, as YYYY-MM-DD (UTC). Range can be at most 366 days.',
          },
          dimension: {
            type: 'string',
            enum: ['Portfolio', 'Program', 'Project', 'StrategicTheme', 'WorkType'],
            description: 'What to group completed work by (default Portfolio).',
          },
          measure: {
            type: 'string',
            enum: ['Count', 'StoryPoints'],
            description:
              'How each completed work item is weighed (default Count). StoryPoints is for point-sized teams only.',
          },
          unestimated: {
            type: 'string',
          ...
          themeCounting: {
            type: 'string',
            enum: ['SplitEvenly', 'CountFully'],
            description:
              'Strategic theme only: how work on a project with multiple effective themes is credited (default SplitEvenly). SplitEvenly divides work evenly so shares add to 100%; CountFully credits all work to each theme (shares sum to > 100%).',
          },
        },
        required: ['idOrCode', 'from', 'to'],
      },
      method: 'get',
      pathTemplate: '/api/organization/teams/{idOrCode}/allocation',
      executionParameters: [
        { name: 'idOrCode', in: 'path' },
        { name: 'from', in: 'query' },
        { name: 'to', in: 'query' },
        { name: 'dimension', in: 'query' },
        { name: 'measure', in: 'query' },
        { name: 'unestimated', in: 'query' },
        { name: 'themeCounting', in: 'query' },
      ],
      requestBodyContentType: undefined,
      securityRequirements: [{ ApiKey: [] }],
    },
  ...
  [
    'TeamsOfTeams_GetList',
    {
      ...
      description: `Get a list of teams of teams in the organization.`,
      inputSchema: {
        type: 'object',
        properties: {
          includeInactive: {
            type: 'boolean',
            description: 'Include inactive teams of teams. Defaults to false.',
          },
        },
      },
      method: 'get',
  ...
  [
    'TeamsOfTeams_GetAllocation',
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          idOrCode: { type: 'string', description: 'Team of teams ID (UUID) or code.' },
          from: {
            type: 'string',
            format: 'date',
            description: 'The first day of completed work to include, as YYYY-MM-DD (UTC).',
      ...
      method: 'get',
      pathTemplate: '/api/organization/teams-of-teams/{idOrCode}/allocation',
      executionParameters: [
        { name: 'idOrCode', in: 'path' },
        { name: 'from', in: 'query' },
        { name: 'to', in: 'query' },
