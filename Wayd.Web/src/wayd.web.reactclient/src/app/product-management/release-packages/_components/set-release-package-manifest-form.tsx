...
import { useSetReleasePackageManifestMutation } from '@/src/store/features/product-management/release-packages-api'
  ...
  const { form, isOpen, isValid, isSaving, handleOk, handleCancel } =
    useModalForm<SetReleasePackageManifestFormValues>({
      onSubmit: async (values: SetReleasePackageManifestFormValues, form) => {
        try {
          // Typed rather than cast through `unknown`: the manifest's version link is the field the
          // double-count rule reads, and a cast here previously hid a rename of it on the API side —
          // the request bound nothing and every line recorded a null version record.
          const request: SetReleasePackageManifestRequest = {
