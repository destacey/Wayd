...
const ReleasesPage: FC = () => {
  ...
  const canImportReleases = hasPermissionClaim('Permissions.Releases.Import')
  ...
  const actions =
    !canCreateRelease && !canImportReleases ? null : (
      <Space>
        {canImportReleases && (
          <Button onClick={() => setOpenImportForm(true)}>Import</Button>
        )}
        {canCreateRelease && (
          <Button onClick={() => setOpenPlanReleaseForm(true)}>
            Add Release
          </Button>
        )}
      </Space>
