...
import { versionActionAvailability } from './version-actions'
...
describe('versionActionAvailability', () => {
  it('offers every move on a planned version', () => {
    // Arrange / Act
    const available = versionActionAvailability(planned())
  ...
  it('refuses a second cut', () => {
    ...
    const available = versionActionAvailability(cut())
  ...
  it('refuses cutting or re-releasing a released version, but still allows withdrawing it', () => {
    ...
    const available = versionActionAvailability(released())
  ...
  it('offers nothing on a withdrawn version', () => {
    // Arrange / Act
    const available = versionActionAvailability(withdrawn())
  ...
  it('refuses moving the target date once a version is done, even with no cut date', () => {
    ...
    // Act
    const available = versionActionAvailability(doneWithoutCut)
  ...
  it('refuses correcting dates on a withdrawn version', () => {
    ...
    // Act
    const available = versionActionAvailability(withdrawnAfterRelease)
  ...
  it('offers reverting only once a version has shipped', () => {
    ...
    expect(versionActionAvailability(released()).canRevert).toBe(true)
    expect(versionActionAvailability(cut()).canRevert).toBe(false)
    expect(versionActionAvailability(planned()).canRevert).toBe(false)
  })
