...
/**
 * Formats a date-time value (a JS `Date` or ISO string) as `MMM D, YYYY hh:mm A`.
 * Returns an empty string for null/undefined.
 */
 ...
 * grid option. Reference from a column with `type: 'dateOnly'` or
 * `type: 'dateTime'`.
 */
export const waydColumnTypes: Record<string, ColTypeDef> = {
  ...
  /**
   * Date-time column. Displays `MMM D, YYYY hh:mm A` while keeping the raw value
   * for sorting. The date filter compares by day, matching the date-only picker
   * that AG Grid provides for date filters.
   */
  dateTime: {
    valueFormatter: (params) => formatDateTime(params.value),
    getQuickFilterText: (params) => formatDateTime(params.value),
    filter: 'agDateColumnFilter',
    filterParams: {
      comparator: (filterDate: Date, cellValue: unknown) => {
        if (!cellValue) return -1
...
export { formatDateOnly, formatDateTime, DATE_ONLY_FORMAT, DATE_TIME_FORMAT }
