...
/// <summary>
/// How a version list orders itself, against SQL Server.
/// </summary>
/// <remarks>
/// Sequence orders one product's versions against each other. It means nothing across products —
...
public sealed class VersionOrderingTests(WaydSqlServerApiFactory factory)
{
        ...
        IDispatcher dispatcher, Guid productId, string version, long? sequence, LocalDate? released)
    {
        ...
        if (released is not null)
        {
            var marked = await dispatcher.Send(
                new MarkVersionReleasedCommand(planned.Value.Id, released.Value),
                TestContext.Current.CancellationToken);
    ...
    public async Task GetVersions_ForOneProduct_OrdersBySequenceWithinTheSameReleasedDate()
    {
        ...
        // Act
        var versions = await dispatcher.Send(
            new GetVersionsQuery(productId), TestContext.Current.CancellationToken);

        // Assert — descending, so the higher sequence leads.
        Assert.Equal([higher, lower], versions.Select(r => r.Id));
    }
    ...
    public async Task GetVersions_AcrossProducts_IgnoresSequence()
    {
        ...
        // Act
        var versions = await dispatcher.Send(
            new GetVersionsQuery(), TestContext.Current.CancellationToken);
    ...
    public async Task GetVersions_PutsUnreleasedFirst()
    {
        ...
        // Assert
        Assert.Equal([plannedVersion, shippedVersion], versions.Select(r => r.Id));
    }

    [Fact]
    public async Task GetVersions_TracksNothing_BecauseItProjects()
    {
