...
**Users created before staging existed.** The `Stage-Entra-First-Sign-In-For-Unlinked-Users` migration stages every Entra user who has never had an identity row, when the provider allows a single tenant. Under a multi-tenant provider it can't choose the tenant; link such a user by converting them to a local account and staging a [cross-provider migration](#cross-provider-migration-oidcoidc-or-waydoidc) back to Microsoft Entra ID.
