...
5. **`JasperFx.Aspire` (AppHost-side) — pull in here, not in Phase 2.** The only official Aspire ↔ Wolverine integration is the `JasperFx.Aspire` package, added to `Wayd.AppHost` (not the service project). It surfaces Wolverine's CLI verbs as .NET Aspire dashboard features and — the part that matters for us — provides **startup gates** (`.WithJasperFxStartup(c => { c.Run("resources", "setup"); c.Run("codegen", "write"); })`, using Aspire's `WaitForCompletion` pattern) plus command buttons on the resource tile (`check-env`, `describe`, `codegen preview`, and the mutating `resources setup` / `codegen write`). It lands cleanly at Stage B because it closes two threads at once:
   - **`resources setup`** provisions the durable inbox/outbox tables (item 3) before the service boots — nothing to provision until Stage B exists.
   - **`codegen write`** pre-generates handler code, which is the path to `TypeLoadMode.Static` and dropping the `WolverineFx.RuntimeCompilation` reference carried since Phase 2 (see Phase 2 task 11).

   Prerequisites: wire `RunJasperFxCommands(args)` into `Wayd.Web.Api`'s `Program.cs` (currently a plain `app.Run()`), and add the package to `Wayd.AppHost`. **Not an observability dependency** — Wolverine already exports OTel traces/metrics to the Aspire dashboard via the registered `"Wolverine"` `ActivitySource` (Phase 2 task 10); `JasperFx.Aspire` only adds the command buttons and startup gates.
