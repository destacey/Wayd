...
| | |
|---|---|
| **Status** | Implemented through the Static codegen flip — Wolverine is the mediator. Phases 0–2, Stage B (durable-outbox plumbing), JasperFx.Aspire wiring, and the Static codegen flip are all done; MediatR is fully removed. Only **Stage C** (selective async routing) remains, and is incremental/ongoing. |
| **Date** | 2026-07-16 (updated through 2026-07-18) |
...
11. Codegen workflow: WolverineFx 6.x **core no longer ships the Roslyn runtime compiler** — with `TypeLoadMode.Dynamic`/`Auto` and no `IAssemblyGenerator` registered, **the host fails to start** (`GH-2876`; invisible to `dotnet build` and unit tests, which never boot the host — only surfaces when the app actually runs). Phase 2 shipped with `WolverineFx.RuntimeCompilation` + `opts.UseRuntimeCompilation()` (`TypeLoadMode.Dynamic` in dev, `Auto` elsewhere). **Superseded — the Static codegen flip (see "Static codegen flip" below) replaced this entirely:** all environments now run `TypeLoadMode.Static` from a committed pre-generated handler tree, and `WolverineFx.RuntimeCompilation` + `UseRuntimeCompilation()` have been dropped.
...
5. **Done: `JasperFx.Aspire` (AppHost-side).** The only official Aspire ↔ Wolverine integration, added to `Wayd.AppHost` (not the service project). It provides **startup gates** (`.WithJasperFxStartup(c => { c.Run("codegen", "write", …); c.Run("resources", "setup", …); })`) plus resource-tile command buttons. `resources setup` is the orchestrated-environment equivalent of the `AddResourceSetupOnStartup()` we wired for plain `dotnet run`/Testcontainers; the `codegen write` gate validates that the built app can generate every handler's code before it takes traffic (it writes under the gate process's build-output content root, so it is a smoke test, not a source refresh). `RunJasperFxCommands(args)` is wired into `Program.cs` behind an args gate (a leading non-`-` arg is a verb; a verbless boot stays `app.Run()`, so `WebApplicationFactory` is unaffected). **Not an observability dependency** — Wolverine already exports OTel to the Aspire dashboard via the `"Wolverine"` `ActivitySource` (Phase 2 task 10).
...
### Static codegen flip ✅ Done (the deferred build-pipeline follow-up)

The Phase 2 / Stage B follow-up carried since the codegen note: pre-generate handler code, run `TypeLoadMode.Static` everywhere, and drop the Roslyn runtime compiler. What landed:

- **Committed handler tree.** `dotnet run -- codegen write` emits ~460 handler `.cs` files (one per handler chain, plus a `GeneratedHandlerRegistry`) into `Wayd.Web.Api/Internal/Generated/WolverineHandlers`. This tree is **committed to the repo** and compiled into `Wayd.Web.Api`. (The estimate was ~890; Wolverine generates per handler *chain*/message, not per handler implementation, and combines/skips some.)
- **`TypeLoadMode.Static` in all environments** (dev included). Static loads the generated `HandlerRegistry` and per-handler types directly — no assembly scan, no Roslyn — so **`WolverineFx.RuntimeCompilation` and `opts.UseRuntimeCompilation()` are dropped** (package reference removed from `Directory.Packages.props` + `Wayd.Infrastructure.csproj`). The trade-off Static imposes: the committed tree is the source of truth and must be regenerated whenever a handler (or a handler dependency's shape) changes.
- **`ApplicationAssembly` must be pinned explicitly** to `Wayd.Web.Api` (passed from `Program.cs` as `typeof(Program).Assembly` into `AddWaydWolverine`). This was the load-bearing gotcha. Static loads the pre-generated types from `WolverineOptions.ApplicationAssembly`, and **neither** of Wolverine's inference strategies yields the right assembly: the caller of `UseWolverine` is `Wayd.Infrastructure` (where the config lives), and `Assembly.GetEntryAssembly()` is the **test-runner** assembly under an integration-test host. The first inference silently degrades to a runtime assembly scan (host still boots — masking the problem); the second throws `ExpectedTypeMissingException` at **first dispatch** (invisible to `dotnet build` and to host-boot; only the integration dispatch tests catch it).
- **Dev inner loop:** a Debug-only `RegenerateWolverineHandlers` post-build target in `Wayd.Web.Api.csproj` reruns `codegen write` after each build (mirrors the existing NSwag target), so a developer's tree self-heals on the next build/run — a one-cycle lag, no manual step.
- **CI freshness guard:** after the Debug build (which regenerates the tree via that target), CI runs a `git diff` on `Internal/Generated/WolverineHandlers` and fails if it changed — catching a tree committed stale (a stale tree = silent wrong/missing dispatch under Static). Codegen output is **idempotent** (byte-identical on repeat), so the guard is reliable.
- **Verification:** `dotnet build` is 0-warning and unit tests pass, but neither exercises this — the guardrails are the real host boot (`WAYD_SKIP_DB_INIT=true`: logs *"code generation mode is Static … Using pre-generated Wolverine HandlerRegistry (458 handler types); skipping assembly scan"*) and the `Wayd.Web.Api.IntegrationTests` dispatch suite (`WolverineConfigurationValidityTests`, `DispatchPipelineTests`, `CrossDomainReplicationTests`, `HangfireIdentityPropagationTests`), which drive real dispatch through the generated code.
...
| Cold-start code generation | Resolved by the Static codegen flip: all environments run `TypeLoadMode.Static` from the committed handler tree (`Wayd.Web.Api/Internal/Generated/WolverineHandlers`), so there is no runtime compilation and `WolverineFx.RuntimeCompilation` is dropped. The tree is kept fresh by a Debug post-build regen target (dev) and a CI `git diff` guard. See "Static codegen flip". |
...
- [x] Schema name for Wolverine envelope tables — **resolved: a dedicated `wolverine` schema (never `dbo`), provisioned by Weasel at startup (Stage B)**.
- [x] Fate of the subscriber-less generic entity events — **resolved: remove the pattern entirely (Phase 0)**.
- [ ] Failure-policy design for Stage C (retry/backoff/DLQ per exception type) — **in progress: first durable candidate is `Project*`→`WorkProject`; see Stage C**.
- [ ] Event classification (inline vs. durable) — **in progress: only `TeamCreatedEvent`→`PlanningTeam` has a hard in-request FK dependency (required cascade FK from `PlanningIntervalTeam`); the rest are async-capable. See Stage C**.
