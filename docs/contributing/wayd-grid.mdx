...
- Headings are not records: row activation, the displayed-rows surfaces (`onDisplayedRowsChange`, `getDisplayedRows`) and the CSV export see only the rows. Flat row reorder turns off while grouped, since the displayed order is no longer the data order.
