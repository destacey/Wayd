...
New connections are **active by default**. Admins can deactivate one from the detail page's action menu — deactivating sets `IsActive=false` and, for syncable connections, also clears `IsSyncEnabled` so the connection is excluded from sync runs. Activating an inactive connection raises an `EntityActivatedEvent`; the deliberate state transition is captured in domain events rather than silently bundled into a config edit.
...
Existing deployments that previously ran employee sync from `SecuritySettings:GraphApi:*` config will, on first boot after the migration, seed a single `EntraConnection` row from `SecuritySettings:AzureAd:{TenantId,ClientId,ClientSecret}` via `EntraConnectionSeeder`. The seeder is insert-only — once the row exists, the database is the source of truth and admins manage the connection via the Settings UI.
