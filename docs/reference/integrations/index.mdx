...
**Sync jobs do not automatically retry.** Their failures are overwhelmingly deterministic — constraint violations, malformed source data, revoked credentials — so a retry would rebuild the identical change set and fail identically while consuming upstream API quota. A failed run goes straight to Hangfire's Failed state, where it stays visible; fix the cause and trigger the sync again. (Transient *transport* faults are handled separately, by the per-connector HTTP resilience pipeline, and never reach the job layer.)
