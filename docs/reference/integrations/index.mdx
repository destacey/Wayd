...
The detail page's **Sync Now** action and **Sync History** tab dispatch by category too, so a future BambooHR / Okta / Jira connector just declares the right `ConnectorCategory` and gets the same plumbing for free.
