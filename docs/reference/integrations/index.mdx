...
**People sync does not automatically retry.** Its failures are overwhelmingly deterministic — constraint violations, malformed source data, revoked credentials — so a retry would rebuild the identical change set and fail identically while consuming upstream API quota. A failed People sync goes straight to Hangfire's Failed state, where it stays visible; fix the cause and trigger the sync again.
