---
title: Organizations
description: Managing teams, teams of teams, operating models, backlogs, dependencies, sprints, risks, and organizational hierarchy in Wayd.
sidebar_position: 1
audience: [user, agent]
---
...
- **[Planning Intervals](../planning/planning-intervals.mdx)** — Teams participate in PIs, committing to [objectives](../planning/planning-intervals.mdx#objectives) and mapping [sprints](../planning/sprints.mdx) to [iterations](../planning/planning-intervals.mdx#iterations-within-a-pi).
- **[Sprints](../planning/sprints.mdx)** — Each team owns sprints that define their time-boxed delivery cadence. Sprint performance is measured with [sprint metrics](../planning/sprints.mdx#sprint-metrics).
...
**Team of Teams** have a simplified detail page focused on hierarchy and risk management. They do not have backlogs or sprints — those belong to the leaf-level delivery teams — but their [Allocation](#allocation-report) report rolls up the completed work of every team beneath them.
...
Operating models have an effective date range, creating a history. When a team changes how it works (e.g., moving from Kanban to Scrum, or relocating to another time zone), a new operating model is added and the previous one is automatically closed the day before the new one starts.

A sprint uses the time zone and grace period of the operating model in effect on its planned start date, for the whole sprint. So a team that moves keeps its old zone for sprints planned before the move, while editing a model corrects every sprint in that model's period. New models are pre-filled from the [scheduling settings](../settings/index.mdx), which never change an existing team.

The operating model affects what the team sees in the UI:
- **Scrum teams** see a **Sprints** section and an **Active Sprint** indicator on their detail page
...
- **Relationships** — Parent Team of Teams and, for a Team of Teams, its child teams
...
- Metric tiles for **Members**, **Backlog**, **Dependencies**, and **Open Risks**, each linking to the section it counts
- **Active Team Sprint** — For Scrum teams, shows the current sprint with timeline progress, completion rate, velocity, in progress count, and average cycle time
- **Members** — A card for each team member showing their name (links to their employee page) and roles. Cards are clickable and navigate to the employee's detail page.
...
Members are managed from the **Actions menu → Team Management**:
- **Add Member** — Select an employee and one or more roles. Employees already on the team are excluded from the selection.
...
The **Sprints** section shows all sprints owned by the team — past, active, and future. Each sprint displays its name, date range, state (Future, Active, Completed), and type.

Sprints are the time-boxed delivery cadence for Scrum teams. They are created in the [Planning](../planning/index.mdx) domain and mapped to [PI iterations](../planning/planning-intervals.mdx#iterations-within-a-pi). See [Sprints](../planning/sprints.mdx) for details on how sprints work and [Sprint Metrics](../planning/sprints.mdx#sprint-metrics) for the metrics tracked per sprint.
...
- Creating new risks directly from the section
...
Memberships can be edited or deleted from this section (with appropriate permissions).

### Activity

The **Activity** section shows the team's recorded history: what changed, who changed it, and when. Teams of Teams carry the same section. See [Activity Log](../activity-log.mdx).
...
Available from the **Reports** menu, the **Backlog Health** report grades the team's open backlog — runway, net flow, WIP load, stale and aging work, readiness gaps near the top of the backlog, carry-over, and work ranked above what it depends on — and lists the work items behind each problem.
...
Available from the **Reports** menu on both teams and teams of teams, the **Allocation** report shows where completed work went over a date range — by portfolio, program, project, strategic theme or work type — and how much of it was linked to no project. On a team of teams it covers every team beneath it, each counted toward the parent it had on the day the work was done.

See [Allocation Report](../work-management/allocation-report.mdx) for what it counts, each measure, and how themes and unestimated work are handled.

## Functional Organization Chart

Wayd renders a **Functional Organization Chart** that visualizes the complete team hierarchy as of a specific date. Navigate to **Organizations > Functional Org Chart** to see how teams are structured across the organization.
...
3. Confirm the removal — all role assignments for that employee are removed
...
5. The team is created with a default operating model (Kanban and Count, with the time zone and grace period from the scheduling settings)
...
3. From the **Actions menu**, click **Team Management > Add Team Membership**
...
3. Select the new **Methodology** and **Sizing Method**, and check the **Time Zone** and **Commitment Grace Period** (pre-filled from the scheduling settings)
...
2. Change the **Methodology**, **Sizing Method**, **Time Zone** or **Commitment Grace Period**

A correction applies to the model's whole period, including past sprints. If the team actually moved to another time zone, set a new operating model from the move date instead.
...
Each employee's detail page includes a **Teams** section showing all teams the employee is currently a member of, with columns for Team name, Team type, and Roles. Team names link to their respective detail pages.
...
- **[Bulk Import](./bulk-import.mdx)** — Loading employees, teams, the team hierarchy, and team staffing from CSV via the import API.
