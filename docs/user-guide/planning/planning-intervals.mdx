...
- **Team Sprints** — Map each team's [sprints](./sprints.mdx) to PI iterations
...
**List View** — Data grid with columns: Key, Name, Is Stretch, Status, Team, Health, Progress (bar), Start Date, Target Date, Order
...
Predictability is only calculated for Active or Completed PIs (not Future). It is shown on the Overview section as a metric with a completed / regular / stretch breakdown, and visualized per-team in the **Team Predictability Radar Chart** and on each team's card in the Teams section.
...
The plan review only shows delivery teams (not Teams of Teams). The selected team is held in the URL as `?team=` (the team code, lowercased), alongside `?section=plan-review` — so a link can point directly at one team's plan.
