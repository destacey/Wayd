...
- **Task** — A specific piece of work under a step. Tasks stack vertically inside the grid cell where their step column meets a swim lane row. A task can also carry a description, a checklist, and a linked work item — see [Task Details](#task-details).
...
Click any goal, step, task, or lane name to edit it in place. **Enter** saves, **Escape** cancels, and clicking away saves. Leaving the field empty keeps the previous name. A task holds more than its title — click the card itself to open [its details](#task-details).
...
Click a task card to open its details. Everything a card cannot show — a description, a checklist, a linked work item — lives here.

The panel is **not modal**. The board stays live beside it: you can keep dragging cards, editing names, and clicking other tasks, and the panel re-points at whichever task you pick. Clicking the open task again closes it, as does the **✕** in the panel header.

On a wide screen the panel opens beside the board and the board narrows to make room, so no column ends up underneath it. On a narrower screen there isn't width to spare, so it slides over the board instead.

| Field | Notes |
|---|---|
| **Title** | The same title shown on the card. **Enter** saves, **Escape** cancels. Leaving it empty keeps the previous title. |
| **Work item** | Links the task to a work item by ID. Once linked, the panel shows the ID with an unlink button beside it. |
| **Swim Lane** | Moves the task to another lane in the same step — the keyboard-friendly alternative to dragging it. |
| **Description** | Free text for anything the title cannot carry. Saves when you click away. |
| **Personas** | The same tags as the card's dots, with names. A tagged persona shows in its own colour; an untagged one is muted. Click to toggle. |
| **Checklist** | Sub-items for work too small to be its own task. |

Title and Description each show a character count while you are typing in them.

### Checklists

A checklist breaks a task into smaller pieces without cluttering the board. The card shows the tally — `2/5` — and the panel holds the items.

Tick an item to mark it done; it greys out and strikes through, and the card's tally updates. Click an item's name to rename it in place. **+ Add item** appends to the list and stays open so you can add several in a row.
...
| **Task** | Reordered within its cell, moved to another step, or moved to another swim lane. A lane change can also be made from the [task details](#task-details) panel, without dragging |
...
1. Click the task card to open its details
2. Add a description for anything the title cannot carry
3. Break the work into checklist items — the card shows the tally, so progress reads from the board
4. Link the work item once the task is picked up, so the map points at the delivery record
