...
| Release package | Its own version, a manifest of component versions | A product of its own; an owning release |
...
Versions are listed with everything not yet shipped first, then those that have shipped, newest first. What is still coming is what you are usually looking for.
...
Both are **whole-set replacements**: the form submits everything the release should end up with, so
a version or package left out is removed from it.

:::warning A version is announced once, by one route
A version already shipping inside one of the release's packages **cannot also be carried directly**,
and Wayd refuses it either way round — adding the version, or adding a package that already contains
it. Otherwise `2026.07` would announce the same shipment twice and "what did it contain" would have
...
Recording a shipment or deployment outcome — cutting, releasing, announcing, withdrawing, correcting
dates, or marking a deployment succeeded, failed or rolled back — additionally requires your account
to be linked to an employee record, because the change is attributed to the person who made it.
