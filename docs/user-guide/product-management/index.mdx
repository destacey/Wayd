...
Product Management is being built in phases. The catalog and delivery records both have screens now.

| Area | Interface | API |
| --- | --- | --- |
| Products, types, tags | Yes | Yes |
| Releases and packages | Yes | Yes |
| Deployments and environments | Yes | Yes |
| Delivery metrics | Partly | Yes |

Delivery metrics reports the two measures this module can compute from what it records. The other two are shown as unavailable, with the reason, rather than approximated — see [Delivery Metrics](./delivery.mdx#delivery-metrics).
