...
| Product dependencies | Yes | Yes |
| Releases and versions | Yes | Yes |
| Release packages | Yes | Yes |
| Deployments and environments | Yes | Yes |
| Delivery overview and version activity | Yes | Yes |
| Rollout — what is running where | Yes | Yes |
| Delivery metrics | Yes | Yes |

Two pages read the records rather than adding to them. **[Delivery](./delivery.mdx#delivery-overview)** measures what was cut and shipped from the version record, which needs no deployments at all. **[Rollout](./delivery.mdx#rollout)** derives what is running in each environment from the deployment record, so it can never drift out of step with it.

Delivery metrics reports the two DORA measures this module can compute from what it records. The other two are shown as unavailable, each with the reason, rather than approximated — see [Delivery Metrics](./delivery.mdx#delivery-metrics).

## Sections

- **[Products](./products.mdx)** — The catalog: creating products, the tree, types, statuses, tags, moving, external links, and the dependencies between products
- **[Delivery](./delivery.mdx)** — Releases, versions, packages, deployments, environments, the rollout and version-activity views, and the delivery metrics computed from them
- **[Bulk Import](./bulk-import.mdx)** — Loading the catalog from CSV through the import API
...
| `Permissions.Products.Update` | Edit, retype, move, tag, change status, link externally, record [dependencies](./products.mdx#dependencies) |
| `Permissions.Products.Delete` | Remove products |
| `Permissions.Products.Import` | [Bulk import](./bulk-import.mdx) products from CSV |
| `Permissions.ProductTypes.*` | Manage the type list |
| `Permissions.ProductTagCategories.*` | Manage the tag axes and the tags on them |
| `Permissions.Delivery.*` | Manage versions, packages and deployments, and [bulk import](./bulk-import.mdx) them |
| `Permissions.Releases.*` | Manage product releases (announcements), and [bulk import](./bulk-import.mdx#releases-import) them |
| `Permissions.DeploymentEnvironments.*` | Manage deployment environments, and [bulk import](./bulk-import.mdx#environments-import) them |
