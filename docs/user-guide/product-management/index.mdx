...
Two pages read the records rather than adding to them. **[Delivery](./delivery.mdx#delivery)** measures what was cut and shipped from the version record, which needs no deployments at all. **[Rollout](./delivery.mdx#rollout)** derives what is running in each environment from the deployment record, so it can never drift out of step with it.
