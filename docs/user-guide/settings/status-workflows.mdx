...
A workflow's detail page carries an **Activity** section holding its recorded history — created or cloned, renamed, statuses added, removed, renamed, reordered and reclassified, published, archived, and assigned to a record type — with who made each change and when. The default workflows Wayd seeds record nothing for their own creation. See [Activity Log](../activity-log.mdx).
