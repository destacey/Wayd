...
1. Open the workflow and choose **Clone**. The copy carries every status and arrives as a Draft.
...
The panel at the top of **Settings → Status Workflows** shows which workflow each record type currently uses. **Change workflow** moves it — and brings every existing record with it.
